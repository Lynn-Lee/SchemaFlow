...
> 依据文档：`docs/SchemaFlow重构优化产品设计与研发计划.md`、`docs/SchemaFlow重构优化工程实施计划.md`、SchemaPilot roadmap dispatcher 执行规则
> 文档定位：给后续自动开发 agent、dispatcher、reviewer 使用的任务编排手册。

## 1. 总目标

本计划把 SchemaFlow 重构拆成可自动派发、可验证、可回滚的小任务队列。它不是新的产品范围扩张，而是把现有产品设计和工程实施计划转换为 Roadmap Dispatcher 可执行的任务协议。

核心目标：

- 先把 SchemaFlow 当前安全、测试和发布基线恢复到可信状态。
...
SchemaPilot 的 roadmap dispatcher 思路不依赖某个单独脚本，而是依赖一组强执行规则。SchemaFlow 自动开发队列采用以下映射：

| SchemaPilot 规则                                | SchemaFlow 任务计划中的落地方式                                                                                          |
| ----------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
...
| 共享 SchemaModel 是权威状态                     | SchemaFlow 对应为 `schema-core` 是 diagram/domain 的权威模型，UI state 不能绕过 command 写入                             |
| 后端 API、共享模型、前端 UI 必须同时更新        | SchemaFlow 首轮没有后端；涉及模型的任务必须同时更新 domain type、storage 映射、UI 调用和 import/export 适配              |
| 所有关键状态变更必须写 AuditLog                 | SchemaFlow OSS Core 对应为 local operation history、backup metadata、migration log；Cloud/Team 预研再定义服务端 AuditLog |
| 高风险变更必须能被 Risk Analyzer 识别           | SchemaFlow 对应为 destructive schema command、SQL export、AI-assisted export 必须返回 warning/risk 标记                  |
| 权限相关功能必须有 negative tests               | Phase 0 到 Phase 7 不做权限；Phase 8 预研文档必须列出 negative test 目录                                              |
| 不允许为了 UI 快速推进绕过服务端权威模型        | SchemaFlow 对应为不允许 UI 直接改多个状态源；所有 diagram 变更走 command 或 repository API                               |
| 不允许在前端保存密钥                            | SchemaFlow Phase 1 硬门禁：前端不得持久化 API key，不得把构建期密钥写入 bundle 或 `/config.js`                           |
...
depends_on: []
owner_lane: baseline
branch: codex/schemaflow-p0-localstorage-test
allowed_files:
...
- `origin` 必须指向 `https://github.com/Lynn-Lee/SchemaFlow.git`。
...
### SCHEMAFLOW-P0-000：Phase 0 执行清单

```yaml
id: SCHEMAFLOW-P0-000
phase: Phase 0
type: PLAN
priority: P0
...
depends_on: []
owner_lane: baseline
branch: codex/schemaflow-p0-plan
allowed_files:
    - docs/阶段验收记录.md
    - docs/SchemaFlow自动开发任务计划.md
verification:
    - rg -n "Phase 0|SCHEMAFLOW-P0" docs
acceptance:
    - Phase 0 每个任务都有责任范围、验收命令和退出标准
    - 阶段验收记录包含 test、build、audit、CI 四类结果
completion:
    - Phase 0 任务卡、退出门槛和验收记录已在 `docs/SchemaFlow自动开发任务计划.md` 与 `docs/阶段验收记录.md` 中补齐。
    - 当前状态已回填为 `done`，避免和计划末尾完成汇总不一致。
```

### SCHEMAFLOW-P0-001：修复测试环境 localStorage 失败
...
### SCHEMAFLOW-P0-002：建立安全依赖基线

```yaml
id: SCHEMAFLOW-P0-002
phase: Phase 0
type: CODE
priority: P0
...
depends_on:
    - SCHEMAFLOW-P0-001
owner_lane: security
branch: codex/schemaflow-p0-dependency-audit
allowed_files:
    - package.json
    - package-lock.json
    - docs/阶段验收记录.md
...
### SCHEMAFLOW-P0-003：补齐 CI 安全门禁

```yaml
id: SCHEMAFLOW-P0-003
phase: Phase 0
type: CODE
priority: P0
...
depends_on:
    ...
    - SCHEMAFLOW-P0-002
owner_lane: release
branch: codex/schemaflow-p0-ci-gates
allowed_files:
...
### SCHEMAFLOW-P0-004：Phase 0 验收记录

```yaml
id: SCHEMAFLOW-P0-004
phase: Phase 0
type: DOC
priority: P0
...
depends_on:
    ...
    - SCHEMAFLOW-P0-003
owner_lane: baseline
branch: codex/schemaflow-p0-acceptance
allowed_files:
    - docs/阶段验收记录.md
verification:
...
completion:
    ...
    - exit_gate: Phase 0 已通过，允许进入 SCHEMAFLOW-P1-000
```
...
### SCHEMAFLOW-P1-000：Phase 1 执行清单

```yaml
id: SCHEMAFLOW-P1-000
phase: Phase 1
type: PLAN
priority: P0
...
depends_on:
    - SCHEMAFLOW-P0-004
owner_lane: security
branch: codex/schemaflow-p1-plan
allowed_files:
    - docs/安全模型与AI边界.md
    - docs/阶段验收记录.md
verification:
...
### SCHEMAFLOW-P1-001：移除构建期和运行时 API key 暴露

```yaml
id: SCHEMAFLOW-P1-001
phase: Phase 1
type: CODE
priority: P0
...
depends_on:
  - SCHEMAFLOW-P1-000
owner_lane: security
branch: codex/schemaflow-p1-remove-browser-keys
allowed_files:
  - Dockerfile
...
result:
  ...
  - 已暂停非 deterministic 的 AI-assisted SQL export fallback，避免在 `SCHEMAFLOW-P1-002` 前把 OpenAI SDK 和 key fallback 打入浏览器产物。
  - 已新增安全回归测试，防止 Docker、Nginx runtime config 和 env module 重新暴露 API key。
```

### SCHEMAFLOW-P1-002：实现 AI mode gating

```yaml
id: SCHEMAFLOW-P1-002
phase: Phase 1
type: CODE
priority: P0
...
depends_on:
    - SCHEMAFLOW-P1-001
owner_lane: ai
branch: codex/schemaflow-p1-ai-mode-gating
allowed_files:
    - src/lib/ai/**
    - src/lib/env.ts
...
### SCHEMAFLOW-P1-003：Note Markdown 安全渲染

```yaml
id: SCHEMAFLOW-P1-003
phase: Phase 1
type: CODE
priority: P0
...
depends_on:
    - SCHEMAFLOW-P1-000
owner_lane: security
branch: codex/schemaflow-p1-safe-markdown
allowed_files:
...
### SCHEMAFLOW-P1-004：Docker 和 Nginx 安全头

```yaml
id: SCHEMAFLOW-P1-004
phase: Phase 1
type: CODE
priority: P1
...
depends_on:
    - SCHEMAFLOW-P1-001
owner_lane: security
branch: codex/schemaflow-p1-nginx-security-headers
allowed_files:
    - default.conf.template
    - Dockerfile
    - docs/部署与安全配置.md
...
### SCHEMAFLOW-P1-005：Phase 1 安全审查

```yaml
id: SCHEMAFLOW-P1-005
phase: Phase 1
type: REVIEW
priority: P0
...
depends_on:
    ...
    - SCHEMAFLOW-P1-004
owner_lane: security
branch: codex/schemaflow-p1-security-review
allowed_files:
    - docs/阶段验收记录.md
verification:
...
### SCHEMAFLOW-P2-000：Phase 2 执行清单

```yaml
id: SCHEMAFLOW-P2-000
phase: Phase 2
type: PLAN
priority: P1
...
depends_on:
    - SCHEMAFLOW-P1-005
owner_lane: core
branch: codex/schemaflow-p2-plan
allowed_files:
...
completion:
    ...
    - 下一项进入 `SCHEMAFLOW-P2-001`，只建立 `src/schema-core/model` 与旧路径 re-export，不修改业务行为。
```

### SCHEMAFLOW-P2-001：建立 schema-core 目录和领域模型出口

```yaml
id: SCHEMAFLOW-P2-001
phase: Phase 2
type: CODE
priority: P1
...
depends_on:
    - SCHEMAFLOW-P2-000
owner_lane: core
branch: codex/schemaflow-p2-schema-core-model
allowed_files:
    - src/schema-core/**
    - src/lib/domain/**
    - src/types/**
...
completion:
    ...
    - 下一项进入 `SCHEMAFLOW-P2-002`，定义 DiagramCommand、CommandResult、CommandContext 和 risk metadata。
```

### SCHEMAFLOW-P2-002：定义 DiagramCommand 基础类型

```yaml
id: SCHEMAFLOW-P2-002
phase: Phase 2
type: CODE
priority: P1
...
depends_on:
    - SCHEMAFLOW-P2-001
owner_lane: core
branch: codex/schemaflow-p2-command-contract
allowed_files:
...
completion:
    ...
    - 本轮不接入 Provider、不迁移业务行为，下一项进入 `SCHEMAFLOW-P2-003`。
```

### SCHEMAFLOW-P2-003：迁移 Table command

```yaml
id: SCHEMAFLOW-P2-003
phase: Phase 2
type: CODE
priority: P1
...
depends_on:
  - SCHEMAFLOW-P2-002
owner_lane: core
branch: codex/schemaflow-p2-table-commands
allowed_files:
  - src/schema-core/commands/**
...
completion:
  ...
  - `SchemaFlowProvider` 的 `addTables`、`updateTable`、`removeTables` 已通过 table command 计算下一状态，同时保留既有 Dexie 写入、事件和旧 undo/redo 数据结构。
  - 下一项进入 `SCHEMAFLOW-P2-004`，迁移 Field、Index、Relationship command。
```

### SCHEMAFLOW-P2-004：迁移 Field、Index、Relationship command

```yaml
id: SCHEMAFLOW-P2-004
phase: Phase 2
type: CODE
priority: P1
...
depends_on:
    - SCHEMAFLOW-P2-003
owner_lane: core
branch: codex/schemaflow-p2-field-index-relation-commands
allowed_files:
    - src/schema-core/commands/**
    - src/context/schemaflow-context/**
    - src/hooks/**
...
completion:
    ...
    - `SchemaFlowProvider` 的 field、index、relationship 入口已接入 command 计算下一状态，并继续沿用既有 Dexie 写入、事件和 undo/redo action；删除 field 的 undo 会恢复被级联移除的 relationship。
    - 下一项进入 `SCHEMAFLOW-P2-005`，迁移 Area、Note、CustomType command。
```

### SCHEMAFLOW-P2-005：迁移 Area、Note、CustomType command

```yaml
id: SCHEMAFLOW-P2-005
phase: Phase 2
type: CODE
priority: P1
...
depends_on:
    - SCHEMAFLOW-P2-004
owner_lane: core
branch: codex/schemaflow-p2-visual-customtype-commands
allowed_files:
    - src/schema-core/commands/**
    - src/context/schemaflow-context/**
    - src/pages/editor-page/**
...
completion:
    ...
    - `SchemaFlowProvider` 的 area、note、custom type 入口已接入 command 计算下一状态，并继续沿用既有 Dexie 写入和 undo/redo action。
    - 下一项进入 `SCHEMAFLOW-P2-006`，接入统一 undo/redo command history。
```

### SCHEMAFLOW-P2-006：接入统一 undo/redo command history

```yaml
id: SCHEMAFLOW-P2-006
phase: Phase 2
type: CODE
priority: P1
...
depends_on:
    - SCHEMAFLOW-P2-005
owner_lane: core
branch: codex/schemaflow-p2-command-history
allowed_files:
    - src/schema-core/commands/**
    - src/context/schemaflow-context/**
    - src/pages/editor-page/**
...
completion:
    ...
    - `RedoUndoAction` 已支持可选 `commandHistory` batch；`SchemaFlowProvider` 中已迁移的 table、field、index、relationship、area、note 和 custom type 操作会在保留旧 undo/redo 行为的同时写入 command metadata。
    ...
    - 下一项进入 `SCHEMAFLOW-P3-000`，定义 storage repository 和 migration 计划。
```
...
### SCHEMAFLOW-P3-000：Phase 3 执行清单

```yaml
id: SCHEMAFLOW-P3-000
phase: Phase 3
type: PLAN
priority: P1
...
depends_on:
    - SCHEMAFLOW-P2-006
owner_lane: storage
branch: codex/schemaflow-p3-plan
allowed_files:
    - docs/storage设计.md
    - docs/阶段验收记录.md
verification:
...
completion:
    ...
    - 已明确 `SCHEMAFLOW-P3-001` 到 `SCHEMAFLOW-P3-004` 的自动开发顺序：先抽 Dexie schema，再抽 repository，再做 diagram transaction，最后做 backup/restore 版本化。
    - 下一项进入 `SCHEMAFLOW-P3-001`，创建 `src/storage/db` 并集中 Dexie schema。
```

### SCHEMAFLOW-P3-001：抽离 Dexie 数据库定义

```yaml
id: SCHEMAFLOW-P3-001
phase: Phase 3
type: CODE
priority: P1
...
depends_on:
    - SCHEMAFLOW-P3-000
owner_lane: storage
branch: codex/schemaflow-p3-dexie-db
allowed_files:
    - src/storage/db/**
...
acceptance:
    ...
    - 不破坏现有本地 diagram 读取
completion:
    - 已新增 `src/storage/db/schemaflow-dexie.ts` 和 `src/storage/db/schema-versions.ts`，集中 `SchemaFlow` Dexie 名称、当前版本 `13`、stores 和既有 upgrade migration。
    - `StorageProvider` 已改为通过 `createSchemaFlowDexie()` 获取已声明 schema 的 typed db instance，CRUD 逻辑仍保留在 Provider，repository 抽离留给 `SCHEMAFLOW-P3-002`。
    - 已新增 `src/storage/db/__tests__/schemaflow-dexie.test.ts`，覆盖数据库名、当前版本和全部当前 store 名称。
    - 下一项进入 `SCHEMAFLOW-P3-002`，抽 Repository API。
```

### SCHEMAFLOW-P3-002：抽 Repository API

```yaml
id: SCHEMAFLOW-P3-002
phase: Phase 3
type: CODE
priority: P1
...
depends_on:
    - SCHEMAFLOW-P3-001
owner_lane: storage
branch: codex/schemaflow-p3-repositories
allowed_files:
...
completion:
    - 新增 `src/storage/repositories/schemaflow-repositories.ts`，集中当前 StorageContext 所需的 config、diagram filter、diagram、table、relationship、dependency、area、custom type 和 note repository API。
    - `StorageProvider` 已收敛为 repository 组合适配层，不再直接访问 Dexie table。
    - 新增 `src/storage/repositories/__tests__/schemaflow-repositories.test.ts`，覆盖 diagram 组合读取、relationship/custom type 排序、diagram id 级联更新、缺失实体返回 undefined，以及 Provider repository 边界。
    - 下一项进入 `SCHEMAFLOW-P3-003`，实现 diagram transaction service。
```

### SCHEMAFLOW-P3-003：实现 diagram transaction service

```yaml
id: SCHEMAFLOW-P3-003
phase: Phase 3
type: CODE
priority: P1
...
depends_on:
    - SCHEMAFLOW-P3-002
owner_lane: storage
branch: codex/schemaflow-p3-diagram-transaction
allowed_files:
...
completion:
    ...
    - `createSchemaFlowRepositories()` 的 diagram add/delete 已接入 transaction service，删除 diagram 会同步清理 tables、relationships、dependencies、areas、custom types、notes 和 diagram filter。
    ...
    - 下一项进入 `SCHEMAFLOW-P3-004`，备份恢复版本化。
```

### SCHEMAFLOW-P3-004：备份恢复版本化

```yaml
id: SCHEMAFLOW-P3-004
phase: Phase 3
type: CODE
priority: P1
...
depends_on:
    - SCHEMAFLOW-P3-003
owner_lane: storage
branch: codex/schemaflow-p3-backup-restore
allowed_files:
    - src/storage/backup/**
    - src/features/**
    - src/pages/**
...
acceptance:
    ...
    - 不兼容文件给出可理解错误
completion:
    - 新增 `src/storage/backup` 版本化 backup contract，当前格式为 `schemaflow.backup` + `schemaVersion: 1`。
    ...
    - 下一项进入 `SCHEMAFLOW-P4-000`，定义 dialect contract 和迁移顺序。
```
...
### SCHEMAFLOW-P4-000：Phase 4 执行清单

```yaml
id: SCHEMAFLOW-P4-000
phase: Phase 4
type: PLAN
priority: P1
...
depends_on:
    - SCHEMAFLOW-P3-004
owner_lane: dialect
branch: codex/schemaflow-p4-plan
allowed_files:
    - docs/方言能力矩阵.md
    - docs/阶段验收记录.md
verification:
...
- 明确迁移顺序：先做 `SCHEMAFLOW-P4-001` common contract，再迁移 PostgreSQL importer，之后补齐其它 dialect wrapper 和 exporter result。
- 下一项进入 `SCHEMAFLOW-P4-001`，创建 dialect contract 和 result type。

### SCHEMAFLOW-P4-001：定义 importer/exporter contract

```yaml
id: SCHEMAFLOW-P4-001
phase: Phase 4
type: CODE
priority: P1
...
depends_on:
    - SCHEMAFLOW-P4-000
owner_lane: dialect
branch: codex/schemaflow-p4-dialect-contract
allowed_files:
    - src/dialects/**
    - src/schema-core/**
...
- 下一项进入 `SCHEMAFLOW-P4-002`，迁移 PostgreSQL importer。

### SCHEMAFLOW-P4-002：迁移 PostgreSQL importer

```yaml
id: SCHEMAFLOW-P4-002
phase: Phase 4
type: CODE
priority: P1
...
depends_on:
    - SCHEMAFLOW-P4-001
owner_lane: dialect
branch: codex/schemaflow-p4-postgresql-importer
allowed_files:
...
- 下一项进入 `SCHEMAFLOW-P4-003`，迁移 MySQL、MariaDB、SQLite、SQL Server 和 Oracle。

### SCHEMAFLOW-P4-003：迁移 MySQL、MariaDB、SQLite、SQL Server、Oracle

```yaml
id: SCHEMAFLOW-P4-003
phase: Phase 4
type: CODE
priority: P1
...
depends_on:
    - SCHEMAFLOW-P4-002
owner_lane: dialect
branch: codex/schemaflow-p4-sql-dialects
allowed_files:
    - src/dialects/mysql/**
...
- 下一项进入 `SCHEMAFLOW-P4-004`，DBML 进入 dialect pipeline。后续全方位优化手册 `SCHEMAFLOW-P-006` 已补齐 CockroachDB 和 ClickHouse dialect wrapper。

### SCHEMAFLOW-P4-004：DBML 进入 dialect pipeline

```yaml
id: SCHEMAFLOW-P4-004
phase: Phase 4
type: CODE
priority: P1
...
depends_on:
    - SCHEMAFLOW-P4-001
owner_lane: dialect
branch: codex/schemaflow-p4-dbml-pipeline
allowed_files:
    - src/dialects/dbml/**
...
- 下一项进入 `SCHEMAFLOW-P4-005`，导入前展示 preview、warnings 和 object counts。

### SCHEMAFLOW-P4-005：导入 Preview Flow

```yaml
id: SCHEMAFLOW-P4-005
phase: Phase 4
type: CODE
priority: P1
...
depends_on:
    ...
    - SCHEMAFLOW-P4-004
owner_lane: ux
branch: codex/schemaflow-p4-import-preview
allowed_files:
    - src/features/import/**
    - src/pages/**
    - src/components/**
...
- 下一项进入 `SCHEMAFLOW-P5-000`，定义 UX 和可访问性验收矩阵。

## 11. Phase 5：产品体验与可访问性

目标：优化核心流程的可理解性、可访问性和设置边界。

### SCHEMAFLOW-P5-000：Phase 5 执行清单

```yaml
id: SCHEMAFLOW-P5-000
phase: Phase 5
type: PLAN
priority: P2
...
depends_on:
    - SCHEMAFLOW-P4-005
owner_lane: ux
branch: codex/schemaflow-p5-plan
allowed_files:
    - docs/可访问性与核心流程验收.md
    - docs/SchemaFlow重构优化产品设计与研发计划.md
    - docs/SchemaFlow重构优化工程实施计划.md
    - docs/SchemaFlow自动开发任务计划.md
    - docs/阶段验收记录.md
verification:
...
本轮结果：已新增 `docs/可访问性与核心流程验收.md`，定义首次进入、Smart Query、导入 Preview、编辑画布和设置中心的桌面、移动、键盘、可访问性与失败状态验收矩阵。Phase 5 后续代码任务必须按该矩阵回归 UI smoke、可访问名称、Dialog 焦点、Disabled 原因和错误提示。下一项进入 `SCHEMAFLOW-P5-001`，重做首次进入入口。

### SCHEMAFLOW-P5-001：重做首次进入入口

```yaml
id: SCHEMAFLOW-P5-001
phase: Phase 5
type: CODE
priority: P2
...
depends_on:
    - SCHEMAFLOW-P5-000
owner_lane: ux
branch: codex/schemaflow-p5-onboarding
allowed_files:
...
本轮结果：已新增 `src/features/onboarding/onboarding-dialog.tsx` 和组件测试。首次无本地 diagram 时不再直接打开旧建图弹窗，而是在编辑页展示 onboarding dialog；首屏提供数据库选择、导入现有数据库、新建空白图、模板示例和 JSON backup 导入入口。未选择数据库或启动方式时继续按钮会显示可理解提示；新建空白图在 `addDiagram` 或 `updateConfig` 失败时回滚 diagram，避免留下半成品。下一项进入 `SCHEMAFLOW-P5-002`，把 Smart Query 拆成结构化向导。

### SCHEMAFLOW-P5-002：Smart Query Wizard

```yaml
id: SCHEMAFLOW-P5-002
phase: Phase 5
type: CODE
priority: P2
...
depends_on:
    - SCHEMAFLOW-P4-005
owner_lane: ux
branch: codex/schemaflow-p5-smart-query-wizard
allowed_files:
    - src/features/import/**
    - src/pages/**
    - src/components/**
...
本轮结果：Smart Query instructions 已展示 `Smart Query Wizard`，明确 SchemaFlow 不要求数据库密码，按选择数据库、复制 Smart Query、粘贴 JSON、Preview tables/relationships/warnings、Confirm import 五步说明流程。导入 preview 解析失败或无可导入对象时，会在粘贴区显示可操作错误，指向 Smart Query JSON、SQL syntax 或 dialect limitations。新增组件测试覆盖无密码原则和五步文案。下一项进入 `SCHEMAFLOW-P5-003`，全局 aria-label 和键盘路径修复。

### SCHEMAFLOW-P5-003：全局 aria-label 和键盘路径修复

```yaml
id: SCHEMAFLOW-P5-003
phase: Phase 5
type: CODE
priority: P2
...
depends_on:
    - SCHEMAFLOW-P5-000
owner_lane: ux
branch: codex/schemaflow-p5-a11y
allowed_files:
    - src/components/**
    - src/pages/**
    - src/features/**
...
本轮结果：已新增 `src/lib/accessibility/__tests__/phase5-a11y-contract.test.ts`，以 Phase 5 契约覆盖核心 icon-only controls 和 Monaco 编辑器用途名称；`CodeSnippet` 的复制/附加动作按钮、Dialog back 按钮、画布 Toolbar、Smart Query/SQL/DBML 编辑区均补充可访问名称。下一项进入 `SCHEMAFLOW-P5-004`，建设设置中心。

### SCHEMAFLOW-P5-004：设置中心

```yaml
id: SCHEMAFLOW-P5-004
phase: Phase 5
type: CODE
priority: P2
...
depends_on:
    ...
    - SCHEMAFLOW-P3-004
owner_lane: ux
branch: codex/schemaflow-p5-settings-center
allowed_files:
...
本轮结果：已新增 `src/features/settings` 设置中心，并通过左侧 Sidebar 的 `Settings` 入口打开。设置中心集中展示 theme、language、minimap、field attributes、scroll action、AI mode、Self-hosted Gateway endpoint/model、导出 backup、恢复 backup 和清理本地 diagram 的危险操作入口；后续 `SCHEMAFLOW-P-005` 已将恢复 backup 入口补齐为先展示 diagram 摘要预览、确认后再写入本地 storage。BYOK key 仍只允许 session-only，不写入 localStorage。`LocalConfigProvider` 已为 localStorage 读写增加 try/catch 容错，浏览器存储不可用时设置中心会显示 session-only 降级说明。下一项进入 `SCHEMAFLOW-P6-000`，定义 Phase 6 性能优化执行清单。

## 12. Phase 6：性能优化

目标：降低首屏 bundle、避免大 schema 阻塞主线程，建立可重复性能基线。

### SCHEMAFLOW-P6-000：Phase 6 执行清单

```yaml
id: SCHEMAFLOW-P6-000
phase: Phase 6
type: PLAN
priority: P2
...
branch: codex/schemaflow-p6-plan
allowed_files:
    - docs/性能基线与优化计划.md
verification:
...
- 下一项进入 `SCHEMAFLOW-P6-001`，仅在代码编辑器路径加载 Monaco，并记录 `npm run build` chunk 变化。

### SCHEMAFLOW-P6-001：Monaco 懒加载

```yaml
id: SCHEMAFLOW-P6-001
phase: Phase 6
type: CODE
priority: P2
...
depends_on:
    - SCHEMAFLOW-P6-000
owner_lane: performance
branch: codex/schemaflow-p6-monaco-lazy
allowed_files:
...
- 下一项进入 `SCHEMAFLOW-P6-002`，模板 lazy registry。

### SCHEMAFLOW-P6-002：模板 lazy registry

```yaml
id: SCHEMAFLOW-P6-002
phase: Phase 6
type: CODE
priority: P2
title: 大模板数据按需加载
...
depends_on:
    - SCHEMAFLOW-P6-000
owner_lane: performance
branch: codex/schemaflow-p6-template-lazy
allowed_files:
    - src/templates-data/**
...
- build 输出已拆出每个模板的独立 JS chunk，例如 `employee-db-romprqJV.js`、`wordpress-db-B8pCH5_j.js`、`monica-db-Bj9lUCcR.js`；入口 `index` 保持约 `2,609.63 kB` / gzip `513.32 kB`，`editor-page` 仍约 `11,472.80 kB` / gzip `1,806.46 kB`，继续进入 `SCHEMAFLOW-P6-003`。
- 下一项进入 `SCHEMAFLOW-P6-003`，Parser 和 layout worker 化。

### SCHEMAFLOW-P6-003：Parser 和 layout worker 化

```yaml
id: SCHEMAFLOW-P6-003
phase: Phase 6
type: CODE
priority: P2
...
depends_on:
    ...
    - SCHEMAFLOW-P6-000
owner_lane: performance
branch: codex/schemaflow-p6-workers
allowed_files:
    - src/workers/**
    - src/features/import/**
    - src/pages/editor-page/**
...
- 下一项进入 `SCHEMAFLOW-P7-000`，定义 Phase 7 发布门禁和文档补齐范围。

## 13. Phase 7：发布治理与文档

目标：让 release、Docker、自托管和贡献流程可重复。

### SCHEMAFLOW-P7-000：Phase 7 执行清单

```yaml
id: SCHEMAFLOW-P7-000
phase: Phase 7
type: PLAN
priority: P2
title: 定义发布门禁和文档补齐范围
...
depends_on:
    - SCHEMAFLOW-P6-003
owner_lane: release
branch: codex/schemaflow-p7-plan
allowed_files:
    - docs/发布检查清单.md
    - docs/阶段验收记录.md
verification:
...
- 同步产品计划、工程实施计划和阶段验收记录；下一项进入 `SCHEMAFLOW-P7-001`，发布 workflow gate。

### SCHEMAFLOW-P7-001：发布 workflow gate

```yaml
id: SCHEMAFLOW-P7-001
phase: Phase 7
type: CODE
priority: P2
...
depends_on:
    - SCHEMAFLOW-P7-000
owner_lane: release
branch: codex/schemaflow-p7-publish-gate
allowed_files:
    - .github/workflows/publish.yaml
    - .github/workflows/ci.yaml
verification:
...
- 下一项进入 `SCHEMAFLOW-P7-002`，补齐工程文档。

### SCHEMAFLOW-P7-002：补齐工程文档

```yaml
id: SCHEMAFLOW-P7-002
phase: Phase 7
type: DOC
priority: P2
...
depends_on:
    - SCHEMAFLOW-P7-001
owner_lane: docs
branch: codex/schemaflow-p7-docs
allowed_files:
    - README.md
    - docs/**
verification:
...
- README 新增文档导航，明确 SchemaFlow 默认本地优先、无账号、无数据库密码，并链接架构、部署安全、AI 隐私、备份恢复、方言能力、导入导出和测试策略文档。
...
- 下一项进入 `SCHEMAFLOW-P7-003`，Issue template 和贡献规则。

### SCHEMAFLOW-P7-003：Issue template 和贡献规则

```yaml
id: SCHEMAFLOW-P7-003
phase: Phase 7
type: DOC
priority: P3
...
depends_on:
    - SCHEMAFLOW-P7-000
owner_lane: docs
branch: codex/schemaflow-p7-issue-templates
allowed_files:
...
本轮结果：已新增 `.github/ISSUE_TEMPLATE/bug_report.yml`、`feature_request.yml`、`dialect_regression.yml`、`security_report.md` 和 `config.yml`，分别约束通用 bug 复现、feature 的本地优先影响、方言 regression fixture、私下安全报告入口和空 issue 禁用策略。`CONTRIBUTING.md` 已补充本地优先边界、PR 验证、dialect regression fixture、security private report 和测试要求。下一项进入 `SCHEMAFLOW-P8-000`，Cloud/Team 边界计划；Phase 8 只做预研文档和接口边界，不新增登录代码。

## 14. Phase 8：可选 Cloud/Team 预研

目标：只定义可选团队版方向，不进入首轮实现。该阶段不会改变 OSS Core 默认无账号体验。

### SCHEMAFLOW-P8-000：Cloud/Team 边界计划

```yaml
id: SCHEMAFLOW-P8-000
phase: Phase 8
type: SPIKE
priority: P3
...
depends_on:
    - SCHEMAFLOW-P7-003
owner_lane: research
branch: codex/schemaflow-p8-cloud-team-research
allowed_files:
...
本轮结果：已新增 `docs/可选账号登录与团队协作预研.md`，明确 OSS Core 不登录仍可完整使用、本地 diagram 不自动上传、云同步必须用户主动开启，并定义 Workspace、Team、权限、AuditLog、租户隔离、数据导出/删除和权限 negative tests 的边界。本轮未新增登录代码，下一项进入 `SCHEMAFLOW-P8-001` 技术栈选型记录。

### SCHEMAFLOW-P8-001：Cloud/Team 技术栈选型记录

```yaml
id: SCHEMAFLOW-P8-001
phase: Phase 8
type: SPIKE
priority: P3
...
depends_on:
    - SCHEMAFLOW-P8-000
owner_lane: research
branch: codex/schemaflow-p8-tech-spike
allowed_files:
    - docs/可选账号登录与团队协作预研.md
verification:
...
本轮结果：已在 `docs/可选账号登录与团队协作预研.md` 补充 `SCHEMAFLOW-P8-001` 技术栈选型记录。推荐方案为 `NestJS + PostgreSQL + Auth.js/OIDC adapter + REST API + append-only sync log`；对比方案为 Fastify 轻量 API，并明确不推荐把 BaaS SDK 作为 OSS Core 默认依赖。文档已说明 OSS Core 隔离方式、Cloud/Team 不作为默认依赖、sync/pull-push 与 conflict 处理边界。本轮未新增登录代码，Phase 8 继续停留在预研和接口 contract。
...
```bash
git switch -c codex/schemaflow-p0-localstorage-test
```
...
`SCHEMAFLOW-P0-001`、`SCHEMAFLOW-P0-002`、`SCHEMAFLOW-P0-003`、`SCHEMAFLOW-P0-004`、`SCHEMAFLOW-P1-000`、`SCHEMAFLOW-P1-001`、`SCHEMAFLOW-P1-002`、`SCHEMAFLOW-P1-003`、`SCHEMAFLOW-P1-004`、`SCHEMAFLOW-P1-005`、`SCHEMAFLOW-P2-000`、`SCHEMAFLOW-P2-001`、`SCHEMAFLOW-P2-002`、`SCHEMAFLOW-P2-003`、`SCHEMAFLOW-P2-004`、`SCHEMAFLOW-P2-005`、`SCHEMAFLOW-P2-006`、`SCHEMAFLOW-P3-000`、`SCHEMAFLOW-P3-001`、`SCHEMAFLOW-P3-002`、`SCHEMAFLOW-P3-003`、`SCHEMAFLOW-P3-004`、`SCHEMAFLOW-P4-000`、`SCHEMAFLOW-P4-001`、`SCHEMAFLOW-P4-002`、`SCHEMAFLOW-P4-003`、`SCHEMAFLOW-P4-004`、`SCHEMAFLOW-P4-005`、`SCHEMAFLOW-P5-000`、`SCHEMAFLOW-P5-001`、`SCHEMAFLOW-P5-002`、`SCHEMAFLOW-P5-003`、`SCHEMAFLOW-P5-004`、`SCHEMAFLOW-P6-000`、`SCHEMAFLOW-P6-001`、`SCHEMAFLOW-P6-002`、`SCHEMAFLOW-P6-003`、`SCHEMAFLOW-P7-000`、`SCHEMAFLOW-P7-001`、`SCHEMAFLOW-P7-002`、`SCHEMAFLOW-P7-003`、`SCHEMAFLOW-P8-000` 和 `SCHEMAFLOW-P8-001` 已完成，Phase 0 和 Phase 1 均通过验收，Phase 2 已建立 schema-core model 出口、command 基础 contract、table command 纯函数、field/index/relationship command 纯函数、area/note/custom type command 纯函数，以及 command history metadata 接入。Phase 3 已完成 storage 执行清单、Dexie schema 集中化、repository API、diagram transaction service 和 backup/restore 版本化。Phase 4 已新增 common dialect contract、PostgreSQL wrapper、MySQL/MariaDB/SQLite/SQL Server/Oracle wrapper、DBML wrapper 和导入 preview flow；unsupported 或降级语义会进入统一 warning/unsupportedObjects，并在用户确认前展示。Phase 5 已建立 UX 和可访问性验收矩阵、首次进入入口、Smart Query Wizard、核心可访问名称修复与设置中心。Phase 6 已新增性能基线与优化计划，完成 Monaco runtime 懒加载拆分、模板 lazy registry、import preview worker 和 area layout worker client。Phase 7 已新增发布检查清单、发布 workflow gate、工程文档导航、issue templates 和贡献规则。Phase 8 已完成 Cloud/Team 边界计划和技术栈选型记录；后续若继续推进，需要另开独立 Cloud/Team 工程计划，不能直接进入登录实现。
