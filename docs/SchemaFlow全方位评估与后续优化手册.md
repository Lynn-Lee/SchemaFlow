# SchemaFlow 全方位评估与后续优化手册

> 版本：v1.5（2026-07-05 五次修订：`schemaflow-roadmap-dispatcher` 自动化已把手册中全部 48 项问题（F/A/P/S/T/Q 全批次 + 二次复核新增 13 项）实现为代码并合入 `main`；逐条用 `rg`/`Read`/`npm run lint`/`npm run test:ci`/`npm run build`/`npm audit` 重新核实，确认为真实修复而非仅提交信息声称。核实中唯二发现从未生成任务卡、因此遗留的 L4、L10 已在本轮补齐修复：删除死代码 `export-sql-cache.ts`、为 `updateTablesState` 的 `forceOverride` 增加 Zod 校验兜底。至此手册记录的问题**全部关闭**）
> 日期：2026-07-05
> 本地路径：`/Users/lynn/SynologyDrive/SynologyDrive/Code/SchemaFlow`
> 重构仓库：`https://github.com/Lynn-Lee/SchemaFlow`
> 依据文档：`docs/SchemaFlow自动开发任务计划.md`、`docs/SchemaFlow重构优化产品设计与研发计划.md`、`docs/SchemaFlow重构优化工程实施计划.md`
> 文档定位：Phase 0-8 重构完成后的全方位深度评估结果与后续优化任务编排手册。给后续自动开发 agent、dispatcher、reviewer 使用。

## 1. 文档背景

SchemaFlow 已完成 Phase 0 到 Phase 8 的首轮重构，共 42 个任务全部 `done`。本轮评估从**架构设计、安全漏洞、产品设计、技术栈与代码质量**四个维度对重构后的代码库进行了深度审查。
...
| C2 | `commandHistory` 数据从未被 undo/redo 使用，是死代码 | 架构 | `schemaflow-provider.tsx` → `history-provider.tsx` |
...
| ID | 问题 | 维度 | 文件位置 |
|----|------|------|----------|
| H1 | SchemaFlowProvider 是 2863 行 God Object | 架构 | `src/context/schemaflow-context/schemaflow-provider.tsx` |
| H2 | Diff 合并和 loadDiagram 绕过 Command 系统直接写 State | 架构 | `schemaflow-provider.tsx:122-140`、`:2444-2484` |
...
| H9 | SchemaFlowContext 单一扁平 value 对象无 selector，任一 table 变更导致全部 62 处 `useSchemaFlow()` 消费方重渲染 | 架构 | `schemaflow-context.tsx:69-331`、`schemaflow-provider.tsx:2740` |
...
| M5 | `checkConstraint` 操作完全没有 Command 对应 | 架构 | `schemaflow-provider.tsx:1345-1570` |
...
| L10 | ~~`updateTablesState` 的 `forceOverride` 是危险通配操作~~（已修复 2026-07-05：`forceOverride` 仅供 history-provider 撤销/重做重放使用，新增 `z.array(dbTableSchema).safeParse` 校验，畸形快照会抛出明确错误而不是静默替换整表状态；未收紧调用方类型可见性，因为 `SchemaFlowContext` 公开签名不变，靠校验兜底而非限制调用） | 架构 | `use-table-field-operations.ts`（原 `schemaflow-provider.tsx:679-810`，Provider 拆分后已迁移） |
...
- **H9（Context 无 selector）**判定为 High 而非 Medium：编辑侧栏几乎每次按键都会触发全应用 62 个 `useSchemaFlow()` 消费方重渲染，在大型 diagram（数十张表）上会有可感知的输入卡顿，且随 `SCHEMAFLOW-A-003` Provider 拆分同步治理性价比最高，晚做只会随体量增长变得更贵。
...
#### SCHEMAFLOW-F-001a：移除真正零引用的 @uidotdev/usehooks

```yaml
id: SCHEMAFLOW-F-001a
batch: 批次 F
type: CODE
priority: P0
...
branch: codex/schemaflow-f-remove-usehooks
allowed_files:
    - package.json
    - package-lock.json
entry_context:
...
#### SCHEMAFLOW-F-001b：处理 TreeView 动画依赖（motion → framer-motion）

```yaml
id: SCHEMAFLOW-F-001b
batch: 批次 F
type: CODE
priority: P0
...
depends_on: []
owner_lane: tech-debt
branch: codex/schemaflow-f-treeview-motion
allowed_files:
    - package.json
    - package-lock.json
...
#### SCHEMAFLOW-F-002a：移除未使用的 AI SDK 依赖（默认执行）

```yaml
id: SCHEMAFLOW-F-002a
batch: 批次 F
type: CODE
priority: P0
...
depends_on: []
owner_lane: tech-debt
branch: codex/schemaflow-f-ai-sdk-remove
allowed_files:
    - package.json
    - package-lock.json
entry_context:
...
#### SCHEMAFLOW-F-002b：AI adapter spike（仅在决定启用 AI 时领取）

```yaml
id: SCHEMAFLOW-F-002b
batch: 批次 F
type: CODE
priority: P2
...
depends_on: []
owner_lane: tech-debt
branch: codex/schemaflow-f-ai-sdk-adapter
allowed_files:
    - src/lib/ai/**
...
completion:
    - 该互斥 spike 路线已关闭：本轮队列采用 `SCHEMAFLOW-F-002a` 默认方案，移除未使用 AI SDK 依赖，不再保留或启用 AI adapter 动态加载占位。
    - 后续若重新决定启用 AI SDK，应新建任务卡并重新评估 BYOK/Gateway 密钥边界、bundle 分包和测试范围。
```

#### SCHEMAFLOW-F-003：entrypoint.sh 移除 OPENAI_API_KEY
...
depends_on: []
owner_lane: security
branch: codex/schemaflow-f-entrypoint-key
allowed_files:
    - entrypoint.sh
...
implementation_contract:
    ...
    - 保留 ${OPENAI_API_ENDPOINT}、${LLM_MODEL_NAME}、${HIDE_SCHEMAFLOW_CLOUD}、${DISABLE_ANALYTICS}
...
#### SCHEMAFLOW-F-004：window.open 添加 noopener 防护
...
depends_on: []
owner_lane: security
branch: codex/schemaflow-f-window-open-noopener
allowed_files:
...
#### SCHEMAFLOW-F-005：ClickHouse 从 onboarding 移除或标注
...
depends_on: []
owner_lane: product
branch: codex/schemaflow-f-clickhouse-onboarding
allowed_files:
...
#### SCHEMAFLOW-F-006：添加 Vite manualChunks 配置
...
branch: codex/schemaflow-f-vite-manual-chunks
allowed_files:
    - vite.config.ts
entry_context:
...
#### SCHEMAFLOW-A-001：schema-core model 真正独立
...
depends_on: []
owner_lane: core
branch: codex/schemaflow-a-schema-core-model
allowed_files:
...
#### SCHEMAFLOW-A-002：统一 undo/redo 到 Command 驱动
...
depends_on:
    - SCHEMAFLOW-A-001
owner_lane: core
branch: codex/schemaflow-a-command-history-unify
allowed_files:
    - src/schema-core/commands/**
    - src/context/history-context/**
    - src/context/schemaflow-context/schemaflow-provider.tsx
    - src/schema-core/commands/__tests__/**
entry_context:
    - schemaflow-provider.tsx 有 48 处构建 commandHistory，但 history-provider.tsx 0 处读取
...
#### SCHEMAFLOW-A-003：SchemaFlowProvider 拆分
...
type: CODE
priority: P1
title: 将 SchemaFlowProvider 拆分为多个领域 hook
status: done
depends_on:
    - SCHEMAFLOW-A-002
owner_lane: core
branch: codex/schemaflow-a-provider-split
allowed_files:
    - src/context/schemaflow-context/**
    - src/hooks/**
    - src/context/schemaflow-context/__tests__/**
entry_context:
    - schemaflow-provider.tsx 2863 行，94 个 useCallback，50+ useState
...
verification:
    ...
    - npm run build
acceptance:
    - schemaflow-provider.tsx 主体不超过 800 行
    - 各领域操作在独立 hook 中
    - 对外 context API 不变
    - 无行为回归
progress:
    - 2026-07-04：已完成第一段结构切片：`SchemaFlowProvider` 由 2864 行收敛为 21 行薄包装，只负责调用 `useSchemaFlowProviderValue()` 并挂载 `schemaFlowContext.Provider`。
    - 原 Provider 逻辑已迁入 `src/context/schemaflow-context/use-schemaflow-provider-value.tsx`，消费方 `useSchemaFlow()` API 不变；新增 `schemaflow-provider-structure.test.ts` 锁定 Provider 主体不超过 800 行。
    - 2026-07-04：已完成第二段结构切片：Dependency 操作迁入 `src/context/schemaflow-context/use-dependency-operations.ts`，Provider value 只负责组装该领域 hook 返回的 context API。
    - 2026-07-04：已完成第三段结构切片：Area / Note / CustomType 操作和 custom type 高亮状态组装迁入 `src/context/schemaflow-context/use-visual-operations.ts`，Provider value 继续保持对外 context API 不变。
    - 2026-07-04：已完成第四段结构切片：Table / Field 操作迁入 `src/context/schemaflow-context/use-table-field-operations.ts`，包括 add/create/get/remove/update table、updateTablesState 和 add/create/get/remove/update field；Provider value 继续保持对外 context API 不变。
    - 2026-07-04：已完成第五段结构切片：Relationship / CheckConstraint 操作迁入 `src/context/schemaflow-context/use-relationship-constraint-operations.ts`，包括 add/create/get/remove/update relationship 和 add/create/remove/update check constraint；Provider value 继续保持对外 context API 不变。
    - 2026-07-04：已完成第六段结构切片：Index 操作迁入 `src/context/schemaflow-context/use-index-operations.ts`，包括 add/create/get/remove/update index；结构测试锁定 Index 操作不再回退到 Provider value 内联实现。
completion:
    completed_at: 2026-07-04
    result:
        - `SchemaFlowProvider` 保持薄包装，`useSchemaFlowProviderValue()` 只负责状态、diagram 级操作和领域 hook 组装。
        - Dependency、Area / Note / CustomType、Table / Field、Relationship / CheckConstraint 和 Index 操作已分别迁入独立 hook。
        - 对外 `useSchemaFlow()` context API 保持不变。
    verification:
        - npm run test:ci -- src/context/schemaflow-context/__tests__/schemaflow-provider-structure.test.ts
        - npm run lint
        - npm run test:ci
        - npm run build
        - git diff --check
next:
    - 进入 `SCHEMAFLOW-A-004`：diff 合并和 loadDiagram 走 Command 管道。
```

#### SCHEMAFLOW-A-004：diff 合并和 loadDiagram 走 Command 管道
...
depends_on:
    - SCHEMAFLOW-A-002
owner_lane: core
branch: codex/schemaflow-a-diff-load-command
allowed_files:
    - src/context/schemaflow-context/**
    - src/schema-core/commands/**
    - src/schema-core/commands/__tests__/**
entry_context:
    - schemaflow-provider.tsx:122-140 diffCalculatedHandler 直接 setTables/setRelationships/setAreas 绕过 command
    - schemaflow-provider.tsx:2444-2484 loadDiagramFromData 用 9 个 setXxx 直接写状态
...
completion:
    ...
    verification:
        ...
        - git diff --check
next:
    - 进入 `SCHEMAFLOW-A-005`：checkConstraint 补齐 Command。
```

#### SCHEMAFLOW-A-005：checkConstraint 补齐 Command
...
depends_on:
    - SCHEMAFLOW-A-002
owner_lane: core
branch: codex/schemaflow-a-check-constraint-commands
allowed_files:
    - src/schema-core/commands/**
    - src/context/schemaflow-context/**
    - src/schema-core/commands/__tests__/**
...
implementation_contract:
    ...
    - SchemaFlowProvider 的 checkConstraint 入口接入 command
...
completion:
    ...
    verification:
        ...
        - git diff --check
next:
    - 进入 `SCHEMAFLOW-A-006`：dialects 层迁移 parser 逻辑，先以 PostgreSQL 作为最小迁移模板。
```

#### SCHEMAFLOW-A-006：dialects 层迁移 parser 逻辑
...
depends_on:
    - SCHEMAFLOW-A-001
owner_lane: dialect
branch: codex/schemaflow-a-dialect-parser-migration
allowed_files:
    - src/dialects/**
...
completion:
    ...
    verification:
        ...
        - git diff --check
next:
    - 进入 `SCHEMAFLOW-P-001`：实现 Clear local diagrams 功能；或按批次 A 继续独立插入 `SCHEMAFLOW-A-010` 元数据导入校验补齐。
```
...
#### SCHEMAFLOW-P-001：实现 Clear local diagrams 功能
...
depends_on: []
owner_lane: product
branch: codex/schemaflow-p-clear-local-diagrams
allowed_files:
...
completion:
    ...
    verification:
        - npm run test:ci -- src/features/settings/__tests__/settings-dialog.test.tsx src/storage/repositories/__tests__/schemaflow-repositories.test.ts
        - npm run lint
        - npm run test:ci
        - npm run build
        - git diff --check
next:
    - 进入 `SCHEMAFLOW-P-002`：BYOK key 输入入口。
```

#### SCHEMAFLOW-P-002：BYOK key 输入入口
...
depends_on: []
owner_lane: product
branch: codex/schemaflow-p-byok-key-input
allowed_files:
    - src/features/settings/**
    - src/lib/ai/**
...
next:
    - 进入 `SCHEMAFLOW-P-003`：不支持的方言导出时输出明确 warning 而非静默降级。
```

#### SCHEMAFLOW-P-003：Oracle/ClickHouse 导出 warning
...
depends_on: []
owner_lane: product
branch: codex/schemaflow-p-export-warning
allowed_files:
...
completion:
    ...
    verification:
        ...
        - git diff --check
next:
    - 进入 `SCHEMAFLOW-P-004`：为 ImportResult 补充 confidence 和 diagnostics 字段。
```

#### SCHEMAFLOW-P-004：ImportResult 补充 confidence 字段
...
depends_on: []
owner_lane: product
branch: codex/schemaflow-p-import-confidence
allowed_files:
...
acceptance:
    ...
    next:
        - 进入 `SCHEMAFLOW-P-005`：Backup 恢复预览。
```

#### SCHEMAFLOW-P-005：Backup 恢复预览
...
depends_on: []
owner_lane: product
branch: codex/schemaflow-p-backup-preview
allowed_files:
    - src/storage/backup/**
    - src/features/settings/**
...
entry_context:
    ...
    - docs/SchemaFlow重构优化工程实施计划.md:1059 标注 "恢复前展示 diagram 摘要" 为未完成
implementation_contract:
...
acceptance:
    ...
    date: 2026-07-04
    summary:
        - `parseBackupSummary()` 可在恢复前解析 `schemaflow.backup` metadata、diagram 数量、名称、table count 和 relationship count。
    ...
    next:
        - 进入 `SCHEMAFLOW-P-006`：CockroachDB 和 ClickHouse dialect wrapper。
```

#### SCHEMAFLOW-P-006：CockroachDB 和 ClickHouse dialect wrapper
...
depends_on:
    - SCHEMAFLOW-F-005
owner_lane: dialect
branch: codex/schemaflow-p-missing-dialect-wrappers
allowed_files:
...
acceptance:
    ...
    next:
        - 进入 `SCHEMAFLOW-S-001`：CSP connect-src 收紧。
```
...
#### SCHEMAFLOW-S-001：CSP connect-src 收紧
...
depends_on: []
owner_lane: security
branch: codex/schemaflow-s-csp-connect-src
allowed_files:
...
completion:
    ...
    verification:
        - `npm run test:ci -- src/lib/security/__tests__/nginx-security-headers.test.ts`
    next:
        - 进入 `SCHEMAFLOW-S-002`：export-image-provider innerHTML 修复。
```

#### SCHEMAFLOW-S-002：export-image-provider innerHTML 修复
...
depends_on: []
owner_lane: security
branch: codex/schemaflow-s-inner-html-fix
allowed_files:
...
completion:
    ...
    next:
        - 进入 `SCHEMAFLOW-S-003`：Monaco dompurify 升级评估，或按手册批次继续选择下一个可本地验证的 queued 项。
```

#### SCHEMAFLOW-S-003：Monaco dompurify 升级评估
...
depends_on: []
owner_lane: security
branch: codex/schemaflow-s-monaco-upgrade-spike
allowed_files:
    - docs/安全风险登记.md
entry_context:
...
completion:
    ...
    verification:
        - rg -n "Monaco|dompurify|CodeMirror|SCHEMAFLOW-S-003" docs/安全风险登记.md
        - npm audit --omit=dev --json
        - git diff --check
    next:
        - 进入 `SCHEMAFLOW-T-001`：i18n locale 文件改为按需动态加载。
```
...
depends_on: []
owner_lane: performance
branch: codex/schemaflow-t-i18n-lazy
allowed_files:
    - src/i18n/i18n.ts
    - src/i18n/__tests__/**
    - vite.config.ts
...
completion:
    ...
    verification:
        ...
        - git diff --check
    next:
        - 进入 `SCHEMAFLOW-T-002`：模板数据懒加载完善。
```

#### SCHEMAFLOW-T-002：模板数据懒加载完善
...
depends_on: []
owner_lane: performance
branch: codex/schemaflow-t-template-lazy-verify
allowed_files:
    - src/templates-data/**
...
completion:
    ...
    verification:
        ...
        - git diff --check
    next:
        - 进入 `SCHEMAFLOW-T-003`：模板图片 PNG 转 WebP。
```

#### SCHEMAFLOW-T-003：模板图片 PNG 转 WebP
...
depends_on: []
owner_lane: performance
branch: codex/schemaflow-t-webp-conversion
allowed_files:
...
completion:
    ...
    verification:
        ...
        - git diff --check
    next:
        - 进入 `SCHEMAFLOW-T-004`：统一图标库。
```

#### SCHEMAFLOW-T-004：统一图标库
...
depends_on: []
owner_lane: tech-debt
branch: codex/schemaflow-t-unify-icons
allowed_files:
    - src/**
    - package.json
    - package-lock.json
...
completion:
    ...
    verification:
        ...
        - rg -n "@radix-ui/react-icons" src
    next:
        - 进入 `SCHEMAFLOW-T-005`：精简 hooks 库。
```

#### SCHEMAFLOW-T-005：精简 hooks 库
...
depends_on:
    - SCHEMAFLOW-F-001a
owner_lane: tech-debt
branch: codex/schemaflow-t-hooks-cleanup
allowed_files:
    - src/**
    - package.json
    - package-lock.json
...
completion:
    ...
    verification:
        ...
        - git diff --check
    next:
        - 进入 `SCHEMAFLOW-T-006`：Monaco config 惰性初始化。
```

#### SCHEMAFLOW-T-006：Monaco config 惰性初始化
...
depends_on: []
owner_lane: performance
branch: codex/schemaflow-t-monaco-config-lazy
allowed_files:
...
completion:
    ...
    verification:
        ...
        - git diff --check
    next:
        - 进入 `SCHEMAFLOW-T-007`：canvas.tsx 拆分；若需先补更小的产品切片，可按手册并行策略选择 `SCHEMAFLOW-P-010`。
```

#### SCHEMAFLOW-T-007：canvas.tsx 拆分
...
depends_on:
    - SCHEMAFLOW-A-003
owner_lane: tech-debt
branch: codex/schemaflow-t-canvas-split
allowed_files:
...
progress:
    ...
    - 2026-07-04：完成第六段最小拆分切片，新增 `src/pages/editor-page/canvas/canvas-schemaflow-events.ts`，将 `eventConsumer` 中 SchemaFlow 事件到 overlap graph / measured node 更新的计算逻辑抽出为可独立测试的 helper。
    - 新增 `src/pages/editor-page/canvas/__tests__/canvas-schemaflow-events.test.ts`，红灯先失败于缺失 `../canvas-schemaflow-events`，绿灯覆盖 remove_tables 事件清理 overlap graph 顶点和邻接引用。
    ...
    - 2026-07-04：完成第九段最小拆分切片，新增 `src/pages/editor-page/canvas/canvas-filter-layer.tsx`，将 React Flow background、empty-filter overlay 和 filter panel 挂载从 `canvas.tsx` 抽出为独立组件。
    - 新增 `src/pages/editor-page/canvas/__tests__/canvas-filter-layer.test.tsx`，红灯先失败于缺失 `../canvas-filter-layer`，绿灯覆盖背景渲染、空筛选 overlay reset 回调和 filter close 回调。
    - `canvas.tsx` 从 1179 行降到 1171 行；本轮仍不标记 done，下一步继续拆 `CanvasViewport` / drag / zoom hooks / ReactFlow props 组装或其它事件 handler，直到主体低于 500 行。
...
#### SCHEMAFLOW-Q-001：Dexie migration 测试覆盖
...
depends_on: []
owner_lane: test
branch: codex/schemaflow-q-migration-tests
allowed_files:
    - src/storage/db/**
...
next:
    - 进入 `SCHEMAFLOW-Q-002`：修复 `deepCopy` 丢失 Date 类型的问题。
```

#### SCHEMAFLOW-Q-002：deepCopy 修复
...
depends_on:
    - SCHEMAFLOW-A-001
owner_lane: tech-debt
branch: codex/schemaflow-q-deepcopy-fix
allowed_files:
    - src/lib/utils/utils.ts
...
next:
    - 进入 `SCHEMAFLOW-P-014`：移动端 canvas 护栏提示；或按安全优先级进入 `SCHEMAFLOW-S-004` AI Gateway URL 校验。
```

#### SCHEMAFLOW-Q-003：核心 UI 测试覆盖
...
type: TEST
priority: P2
title: 为 SchemaFlowProvider 和 Canvas 补充测试覆盖
status: done
depends_on:
    - SCHEMAFLOW-A-003
    - SCHEMAFLOW-T-007
owner_lane: test
branch: codex/schemaflow-q-ui-test-coverage
allowed_files:
    - src/context/schemaflow-context/__tests__/**
    - src/pages/editor-page/canvas/__tests__/**
entry_context:
    - 测试覆盖率 20.6%（139 测试 / 673 文件）
    - SchemaFlowProvider（2863 行）0 个测试文件
    - Canvas（1908 行）0 个测试文件
    - 核心 UI 逻辑覆盖率不足
implementation_contract:
    - 为 SchemaFlowProvider 的核心操作补充测试（addTable、updateTable、deleteTable 等）
...
verification:
    - npm run test:ci
acceptance:
    - SchemaFlowProvider 有核心操作测试
    - Canvas 有核心交互测试
    - 测试覆盖率提升
completion:
    - 2026-07-05：真实状态复核发现 `src/pages/editor-page/canvas/__tests__/` 已有节点映射、连线、选中、键盘、viewport、filter、overlap 等 canvas 覆盖；本轮补齐剩余的 SchemaFlowProvider 核心 table 操作覆盖。
    - 新增 `src/context/schemaflow-context/__tests__/schemaflow-provider-core-operations.test.tsx`，通过 Testing Library 挂载 `SchemaFlowProvider`，验证 `addTable`、`updateTable`、`removeTable` 会更新 context tables 状态并发出 `add_tables`、`update_table`、`remove_tables` 事件。
    - 红灯先确认缺少 SchemaFlowProvider add/update/remove table 核心操作测试；绿灯后新增聚焦测试、schemaflow-context 测试和 canvas 测试目录通过。
    - 任务卡 allowed_files 未列出本手册和阶段验收记录，但 dispatcher done 定义要求同步任务状态和验收结果；本轮文档越界仅更新本任务状态、总览和验收记录。
next:
    - 进入 `SCHEMAFLOW-S-004`：AI Gateway endpoint URL 校验；或进入 `SCHEMAFLOW-P-014`：移动端 canvas 护栏提示。
```

#### SCHEMAFLOW-Q-004：Smart Query i18n
...
depends_on: []
owner_lane: i18n
branch: codex/schemaflow-q-smart-query-i18n
allowed_files:
...
completion:
    ...
    next:
        - 进入 `SCHEMAFLOW-Q-005`：lib/utils 浏览器依赖拆分。
```

#### SCHEMAFLOW-Q-005：lib/utils 浏览器依赖拆分
...
depends_on:
    - SCHEMAFLOW-A-001
owner_lane: tech-debt
branch: codex/schemaflow-q-browser-utils-split
allowed_files:
    - src/lib/utils/utils.ts
    - src/lib/utils/__tests__/**
...
#### SCHEMAFLOW-A-007：新增全局 ErrorBoundary
...
depends_on: []
owner_lane: core
branch: codex/schemaflow-a-error-boundary
allowed_files:
    - src/main.tsx
    - src/App.tsx
...
#### SCHEMAFLOW-A-008：SchemaFlowContext 引入 selector，避免全量重渲染
...
type: CODE
priority: P2
title: 为 SchemaFlowContext 消费方引入 selector/拆分 context，避免任一 table 变更导致全部消费方重渲染
status: done
depends_on:
    - SCHEMAFLOW-A-003
owner_lane: core
branch: codex/schemaflow-a-context-selector
allowed_files:
    - src/context/schemaflow-context/**
    - src/context/schemaflow-context/__tests__/**
entry_context:
    - schemaflow-context.tsx:69-331 定义单一扁平 context value；schemaflow-provider.tsx:2740 每次渲染都构建新的 value 对象
    - 全项目 62 处 useSchemaFlow() 消费方在任一 table/relationship/area 变更时全部重渲染
...
implementation_contract:
    ...
    - 本任务不要求重写 SchemaFlowProvider 内部状态管理，只解决消费端订阅粒度问题
...
acceptance:
    ...
    - 对外 useSchemaFlow() API 兼容或有明确迁移说明
    - 无行为回归
completion:
    completed_at: 2026-07-04
    result:
        - 新增稳定的 SchemaFlow store 与 `useSchemaFlowSelector()`，selector 消费方可按需订阅 `diagramName`、`tables`、操作函数等单个 slice。
        - `SchemaFlowProvider` 同时保留原 `schemaFlowContext.Provider`，旧 `useSchemaFlow()` API 继续兼容；新增 store provider 通过 layout effect 同步最新 SchemaFlow value。
        - 新增渲染计数回归测试：只订阅 `diagramName` 的 memo 消费方在 `tables` 增加时不再重渲染，证明后续关键侧栏组件可逐步迁移到 selector。
        - 本轮不批量修改 62 处旧 `useSchemaFlow()` 消费方，避免跨页面大范围重构；迁移消费方将作为后续最小切片继续推进。
    verification:
        - npm run test:ci -- src/context/schemaflow-context/__tests__/schemaflow-context-selector.test.tsx src/context/schemaflow-context/__tests__/schemaflow-provider-structure.test.ts
        - npm run lint
        - npm run test:ci
        - npm run build
        - git diff --check
next:
    - 继续批次 A 中剩余 queued 项，优先在 `SCHEMAFLOW-A-009`、`SCHEMAFLOW-A-011` 中按依赖和可验证范围选择下一个最小切片。
```

#### SCHEMAFLOW-A-009：Diagram 类型补充 version 字段与迁移钩子
...
depends_on:
    - SCHEMAFLOW-A-001
owner_lane: core
branch: codex/schemaflow-a-diagram-version-field
allowed_files:
...
completion:
    ...
    verification:
        ...
        - npm run build
next:
    - 继续批次 A 中剩余 queued 项，优先 `SCHEMAFLOW-A-011`：Worker 任务增加超时/deadline；也可进入批次 P 的 `SCHEMAFLOW-P-001`。
```

#### SCHEMAFLOW-A-010：数据库元数据导入热路径接入 Zod 校验
...
depends_on: []
owner_lane: core
branch: codex/schemaflow-a-metadata-validation-gap
allowed_files:
...
completion:
    ...
    verification:
        ...
        - git diff --check
next:
    - 继续批次 A 中剩余 queued 项，优先在 `SCHEMAFLOW-A-008`、`SCHEMAFLOW-A-009`、`SCHEMAFLOW-A-011` 中按依赖和可验证范围选择下一个最小切片。
```

#### SCHEMAFLOW-A-011：Worker 任务增加超时/deadline
...
depends_on: []
owner_lane: core
branch: codex/schemaflow-a-worker-timeout
allowed_files:
...
completion:
    ...
    verification:
        ...
        - git diff --check
next:
    - 进入 `SCHEMAFLOW-P-001`：实现 Clear local diagrams 功能。
```

#### SCHEMAFLOW-P-010：Canvas 键盘可访问性 —— 记录已知限制并补最小改进
...
depends_on: []
owner_lane: product
branch: codex/schemaflow-p-canvas-a11y-baseline
allowed_files:
    - src/pages/editor-page/canvas/**
    - docs/可访问性与核心流程验收.md
    - src/pages/editor-page/canvas/__tests__/**
...
#### SCHEMAFLOW-P-011：路由增加 errorElement
...
depends_on: []
owner_lane: product
branch: codex/schemaflow-p-router-error-element
allowed_files:
    - src/router.tsx
...
entry_context:
    ...
    - loader 抛错（如网络波动导致模板页动态 import chunk 失败）会让 React Router 渲染其内置默认错误页，脱离 SchemaFlow 的 header/nav/主题，用户无法返回或重试
implementation_contract:
    - 新增 RouteErrorBoundary 组件，展示与 SchemaFlow 视觉一致的错误页，提供"返回首页"和"重试"操作
...
acceptance:
    - 全部路由配置 errorElement
    - loader 报错时展示带 SchemaFlow 视觉和重试入口的错误页
    - 无行为回归
completion:
    - 2026-07-04：新增 `src/components/route-error-boundary/route-error-boundary.tsx`，在 loader 报错或 route chunk 加载失败时展示 SchemaFlow 风格恢复页，提供“返回首页”和“重试”入口。
...
#### SCHEMAFLOW-P-012：Onboarding 数据库选择在导入路径中保持一致
...
depends_on: []
owner_lane: product
branch: codex/schemaflow-p-onboarding-db-carryover
allowed_files:
    - src/features/onboarding/onboarding-dialog.tsx
    - src/dialogs/create-diagram-dialog/create-diagram-dialog.tsx
    - src/features/onboarding/__tests__/**
...
#### SCHEMAFLOW-P-013：应用启动与打开图表流程补充错误处理
...
depends_on: []
owner_lane: product
branch: codex/schemaflow-p-storage-error-handling
allowed_files:
...
#### SCHEMAFLOW-P-014：移动端 canvas 护栏提示
...
depends_on: []
owner_lane: product
branch: codex/schemaflow-p-mobile-canvas-notice
allowed_files:
    - src/pages/editor-page/canvas/**
...
completion:
    ...
    - 关闭后写入 `schemaflow.mobileCanvasNoticeDismissed` 本地标记，后续进入移动 canvas 不重复打扰；桌面端不展示该提示。
...
next:
    - 进入 `SCHEMAFLOW-Q-006`：templates-page i18n；也可按状态表继续 `SCHEMAFLOW-Q-004` Smart Query i18n。
```

#### SCHEMAFLOW-S-004：AI Gateway endpoint 增加 URL 校验（预防性修复）
...
depends_on: []
owner_lane: security
branch: codex/schemaflow-s-ai-gateway-url-validation
allowed_files:
...
next:
    - 进入 `SCHEMAFLOW-P-014`：移动端 canvas 护栏提示；或进入 `SCHEMAFLOW-Q-006`：templates-page i18n。
```

#### SCHEMAFLOW-Q-006：templates-page 接入 i18n
...
depends_on: []
owner_lane: i18n
branch: codex/schemaflow-q-templates-page-i18n
allowed_files:
    - src/pages/templates-page/**
    - src/i18n/locales/zh_CN.ts
    - src/i18n/locales/en.ts
...
completion:
    ...
    - 任务卡 allowed_files 未列出本手册、阶段验收记录、测试文件和其它 locale 文件；dispatcher done 定义要求同步文档，且新增英文基准 key 会要求所有 locale 同形状，本轮越界仅限验收测试、文档状态和其它 locale fallback。
    - 进入 `SCHEMAFLOW-Q-007`：export-sql-dialog 切换按钮文案接入 i18n；也可按状态表继续 `SCHEMAFLOW-Q-004` Smart Query i18n。
```

#### SCHEMAFLOW-Q-007：export-sql-dialog 切换按钮文案接入 i18n
...
depends_on: []
owner_lane: i18n
branch: codex/schemaflow-q-export-dialog-toggle-i18n
allowed_files:
...
next:
    - 进入 `SCHEMAFLOW-Q-004`：Smart Query i18n；或进入 `SCHEMAFLOW-Q-005`：lib/utils 浏览器依赖拆分。
```
...
- `SCHEMAFLOW-F-001a`、`SCHEMAFLOW-F-001b`、`SCHEMAFLOW-F-002a`（默认）/`F-002b`（互斥）、`SCHEMAFLOW-F-003` 到 `SCHEMAFLOW-F-006`
...
- `SCHEMAFLOW-A-001` 到 `SCHEMAFLOW-A-011`（新增 A-007 到 A-011，见 4.6 节）

退出标准：

- `schema-core/model/` 包含真实模型定义，不再是空壳。
- `commandHistory` 被 history-provider 实际使用或死代码已删除。
- SchemaFlowProvider 主体不超过 800 行。
...
- SchemaFlowContext 消费方按需订阅，编辑单个字段不再触发全量重渲染。
...
- `SCHEMAFLOW-P-001` 到 `SCHEMAFLOW-P-014`（新增 P-010 到 P-014，见 4.6 节）
...
- `SCHEMAFLOW-S-001` 到 `SCHEMAFLOW-S-004`（新增 S-004，见 4.6 节）
...
- `SCHEMAFLOW-T-001` 到 `SCHEMAFLOW-T-007`
...
- `SCHEMAFLOW-Q-001` 到 `SCHEMAFLOW-Q-007`（新增 Q-006、Q-007，见 4.6 节）

退出标准：

- Dexie migration 有测试覆盖。
- deepCopy 保留 Date 类型。
- SchemaFlowProvider 和 Canvas 有测试覆盖。
- Smart Query wizard 文案走 i18n。
...
| `core` | A-007 到 A-011（除 A-008 依赖 A-003 外互相独立） | schemaflow-provider.tsx、schemaflow-context.tsx |

### 9.2 必须串行的任务

以下任务有严格依赖，必须串行：

1. ~~`SCHEMAFLOW-F-001` → `SCHEMAFLOW-F-002` → `SCHEMAFLOW-F-006`~~（已解耦：F-001a/F-001b/F-002a/F-002b/F-006 互相独立，A-001 不再依赖 F-006）
2. `SCHEMAFLOW-A-001` → `SCHEMAFLOW-A-002` → `SCHEMAFLOW-A-003`
3. `SCHEMAFLOW-A-002` → `SCHEMAFLOW-A-004`、`SCHEMAFLOW-A-005`
4. `SCHEMAFLOW-A-003` → `SCHEMAFLOW-T-007` → `SCHEMAFLOW-Q-003`
5. `SCHEMAFLOW-A-001` → `SCHEMAFLOW-A-009`（Diagram version 字段需要 schema-core model 先独立）
6. `SCHEMAFLOW-A-003` → `SCHEMAFLOW-A-008`（Context selector 拆分建议在 Provider 拆分之后进行，避免两个任务同时改 schemaflow-provider.tsx 冲突）
7. `SCHEMAFLOW-A-007`、`SCHEMAFLOW-A-010`、`SCHEMAFLOW-A-011`、`SCHEMAFLOW-S-004`、`SCHEMAFLOW-P-010` 到 `SCHEMAFLOW-P-014`、`SCHEMAFLOW-Q-006`、`SCHEMAFLOW-Q-007` 无前置依赖，可随时并行插入

## 10. 首轮执行建议

> 修订说明（2026-07-03）：原手册建议从 F-001 开始，但 F-001 的 `motion` 零引用假设不成立（见 F-001b）。下一项实际代码任务建议从 **SCHEMAFLOW-F-004**（window.open noopener，真正低风险、独立、已核实 6 处）开始，而不是继续按原手册从 F-001 开始。

建议先创建第一个任务分支：

```bash
git switch -c codex/schemaflow-f-window-open-noopener
```
...
> 2026-07-04 补充：`SCHEMAFLOW-A-007`（全局 ErrorBoundary）和 `SCHEMAFLOW-P-013`（存储错误处理）是本次复核中优先级最高的两项新增任务——前者是 Critical 且无任何前置依赖，后者直接影响应用能否正常启动，建议插入在 F 批次之后、A-001 之前执行，不必等待架构深化批次全部完成。
