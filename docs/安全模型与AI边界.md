...
| 任务 | 目标 | 关键验收 |
| --- | --- | --- |
| `CHARTDB-P1-001` | 移除 Dockerfile 和 `/config.js` 中的 API key 暴露 | 构建产物和运行时配置不输出 `OPENAI_API_KEY` |
| `CHARTDB-P1-002` | 增加 Disabled、BYOK Session、Gateway 三种 AI mode | 默认 Disabled，BYOK 不持久化密钥，AI 请求前有隐私确认 |
