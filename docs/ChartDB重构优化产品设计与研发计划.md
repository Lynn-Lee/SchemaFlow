...
当前状态：`CHARTDB-P5-000` 已新增 `docs/可访问性与核心流程验收.md`，定义首次进入、Smart Query、导入 Preview、编辑画布和设置中心的桌面、移动、键盘、可访问性与失败状态验收矩阵。后续 Phase 5 代码任务必须按该矩阵回归可访问名称、Dialog 焦点、Disabled 原因、错误提示和移动端首屏主动作。下一项进入 `CHARTDB-P5-001`，重做首次进入入口。
