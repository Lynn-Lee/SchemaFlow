...
当前状态：`CHARTDB-P6-000` 已新增 `docs/性能基线与优化计划.md`，明确首屏 JS bundle、Monaco chunk、模板数据 chunk、large schema import 时间、layout worker 和 CI 性能预算的基线记录方式。`CHARTDB-P6-001` 已把 `CodeSnippet` 外层和 DBML highlight helper 的 Monaco runtime 静态依赖移入懒加载编辑器路径，`npm run build` 中 `code-editor` chunk 从 `15,279.59 kB` / gzip `2,787.98 kB` 降至 `3,788.30 kB` / gzip `976.92 kB`。剩余 `editor-page` 大 chunk 主要进入 DBML/parser 与 worker 化治理。下一项进入 `CHARTDB-P6-002`，建设模板 lazy registry。
