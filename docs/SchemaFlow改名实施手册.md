...
**Goal:** 将当前 SchemaFlow 项目整体改名为 SchemaFlow，并让代码、文档、构建产物、运行时配置和用户可见文案统一使用 SchemaFlow。
...
- `SchemaFlow|schemaflow|SCHEMAFLOW` 当前命中 199 个文件，共 4397 处：
  - `SchemaFlow`：880 处
  - `schemaflow`：2469 处
  - `SCHEMAFLOW`：1048 处
- 命中范围：
...
| 当前写法 | 新写法 | 用途 |
| --- | --- | --- |
| `SchemaFlow` | `SchemaFlow` | 产品名、React 类型名、函数名、文档正文 |
| `schemaflow` | `schemaflow` | package name、文件名、URL path、storage key、Docker tag |
| `SCHEMAFLOW` | `SCHEMAFLOW` | env var、常量、任务 ID、CI 变量 |
| `SchemaFlowProvider` | `SchemaFlowProvider` | React provider |
| `useSchemaFlow` | `useSchemaFlow` | React hook |
| `SchemaFlowContext` | `SchemaFlowContext` | React context type |
| `SchemaFlowConfig` | `SchemaFlowConfig` | config domain type |
| `createSchemaFlowDexie` | `createSchemaFlowDexie` | Dexie factory |
| `schemaflow.backup` | `schemaflow.backup` | 新 backup 格式标识 |
| `schemaflow-local` | `schemaflow-local` | 新 backup source |

执行原则：

- 用户可见文案、正式文档、包名、标题、SEO、下载文件名和新导出的 backup 必须使用 SchemaFlow。
- 旧运行时契约只允许以 `legacy` 兼容常量存在，并且只能在迁移、解析旧数据或回归测试中出现。
- 若后续要求仓库内完全零 `SchemaFlow|schemaflow|SCHEMAFLOW` 字符串，则会牺牲旧 IndexedDB、旧 localStorage、旧 backup 文件兼容；默认不采用这种破坏性模式。
...
| 契约 | 当前值 | 新值 | 兼容要求 |
| --- | --- | --- | --- |
| Dexie database name | `SchemaFlow` | `SchemaFlow` | 首次启动时检测旧库，迁移 diagrams、config、filters 等 store 到新库。迁移成功后保留旧库，不自动删除。 |
| Backup format | `schemaflow.backup` | `schemaflow.backup` | 新导出使用 `schemaflow.backup`；恢复入口同时接受旧 `schemaflow.backup`。 |
| Backup source | `schemaflow-local` | `schemaflow-local` | 新导出使用新 source；旧备份解析后统一映射为兼容 summary。 |
| AI local config | `schemaflow.ai.mode` 等 | `schemaflow.ai.mode` 等 | 初始化先读新 key，若为空再读旧 key 并写入新 key。 |
| BYOK/session tests | `schemaflow.ai.byok.key` | `schemaflow.ai.byok.key` | 测试同时覆盖旧 key 不再作为主写入位置。 |
| Canvas notice key | `schemaflow.mobileCanvasNoticeDismissed` | `schemaflow.mobileCanvasNoticeDismissed` | 双读一次，写入新 key。 |
| Runtime env | `HIDE_SCHEMAFLOW_CLOUD` / `VITE_HIDE_SCHEMAFLOW_CLOUD` | `HIDE_SCHEMAFLOW_CLOUD` / `VITE_HIDE_SCHEMAFLOW_CLOUD` | Nginx runtime config 和 Vite build config 先支持新旧两个变量，新变量优先。 |
| Docker smoke name/tag | `schemaflow-release-smoke` | `schemaflow-release-smoke` | GitHub Actions、README 和 publish workflow 一起改。 |
...
schemaflow -> schemaflow
SCHEMAFLOW -> SCHEMAFLOW
https://schemaflow.io -> 新 SchemaFlow 官网域名
https://app.schemaflow.io -> 新 SchemaFlow app 域名
https://docs.schemaflow.io -> 新 SchemaFlow docs 域名
https://github.com/schemaflow/schemaflow -> 新 SchemaFlow 上游或 Lynn-Lee/SchemaFlow 仓库
ghcr.io/schemaflow/schemaflow -> 新 SchemaFlow container image
support@schemaflow.io -> 新 SchemaFlow support 邮箱
```
...
```bash
rg -n "SchemaFlow|schemaflow.io|app.schemaflow.io|docs.schemaflow.io|github.com/schemaflow|ghcr.io/schemaflow|support@schemaflow.io" README.md CONTRIBUTING.md CODE_OF_CONDUCT.md CLA.md index.html .github
```
...
export const HIDE_SCHEMAFLOW_CLOUD: boolean =
    (window?.env?.HIDE_SCHEMAFLOW_CLOUD ??
        ...
        import.meta.env.VITE_HIDE_SCHEMAFLOW_CLOUD) === 'true';
```

调用方从 `HIDE_SCHEMAFLOW_CLOUD` 改为 `HIDE_SCHEMAFLOW_CLOUD`。

- [ ] **Step 2: 更新 Docker build args 和 Nginx config**

`Dockerfile` 使用 `VITE_HIDE_SCHEMAFLOW_CLOUD`，并在兼容期接受旧 build arg。

`entrypoint.sh` 和 `default.conf.template` 输出 `HIDE_SCHEMAFLOW_CLOUD`，同时可从旧 `HIDE_SCHEMAFLOW_CLOUD` 回退。

- [ ] **Step 3: 验证构建配置**

```bash
rg -n "HIDE_SCHEMAFLOW_CLOUD|VITE_HIDE_SCHEMAFLOW_CLOUD" Dockerfile default.conf.template entrypoint.sh src/lib/env.ts docs/部署与安全配置.md
...
export const LEGACY_SCHEMAFLOW_DATABASE_NAME = 'SchemaFlow';
```

`createSchemaFlowDexie()` 默认打开 `SchemaFlow`。迁移函数检测旧 `SchemaFlow` 库，按当前 `SCHEMAFLOW_STORE_NAMES` 对应的新 `SCHEMAFLOW_STORE_NAMES` 复制数据。迁移失败时不删除旧库，并抛出可诊断错误。
...
错误文案从 `Invalid SchemaFlow backup file` 改为 `Invalid SchemaFlow backup file`。
...
- 旧 `schemaflow.backup` fixture 仍可恢复。
...
旧 `schemaflow/schemaflow` star iframe、`schemaflow.io` 页面链接和 `@SchemaFlow_io` Twitter metadata 改为 SchemaFlow 对应值。
...
**Files:**
- Move: `docs/SchemaFlow自动开发任务计划.md` -> `docs/SchemaFlow自动开发任务计划.md`
- Move: `docs/SchemaFlow重构优化产品设计与研发计划.md` -> `docs/SchemaFlow重构优化产品设计与研发计划.md`
- Move: `docs/SchemaFlow重构优化工程实施计划.md` -> `docs/SchemaFlow重构优化工程实施计划.md`
- Move: `docs/SchemaFlow全方位评估与后续优化手册.md` -> `docs/SchemaFlow全方位评估与后续优化手册.md`
- Modify: `AGENTS.md`
...
将正式项目名和文档路径改为 SchemaFlow。任务 ID 从 `SCHEMAFLOW-*` 改为 `SCHEMAFLOW-*`。
...
`schemaflow-release-smoke` 改为 `schemaflow-release-smoke`。
...
```bash
rg -n "SchemaFlow|schemaflow|SCHEMAFLOW" --glob '!node_modules/**' --glob '!dist/**' --glob '!.git/**' --glob '!.codegraph/**'
```
...
- 老用户的 `SchemaFlow` IndexedDB、`schemaflow.ai.*` 设置、`schemaflow.backup` 文件可被读取或迁移。
...
4. `refactor: rename SchemaFlow React context to SchemaFlow`
...
- 远端仓库从 `SchemaFlow` 改为 `SchemaFlow` 需要先创建新 GitHub repo；在确认前不得自行改 remote。
- `CHANGELOG.md` 和历史 commit/issue 链接属于历史记录。若要求严格零旧品牌，也要替换；若保留历史真实性，则应加入 legacy allowlist。当前需求按“全部正式文档改名”为目标，历史链接在执行前需要最终确认。
- `schemaflow.io`、`app.schemaflow.io`、`docs.schemaflow.io`、`support@schemaflow.io` 没有已知 SchemaFlow 真实域名和邮箱；确认真实域名和邮箱前，外部链接改名保持 blocked。
