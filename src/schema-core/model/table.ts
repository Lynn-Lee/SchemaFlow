import { dbIndexSchema, type DBIndex } from './table-index';
import { dbFieldSchema, type DBField } from './field';
import type { DBRelationship } from './relationship';
import {
    dbCheckConstraintSchema,
    type DBCheckConstraint,
} from './check-constraint';
import { schemaNameToDomainSchemaName } from './schema';
import { z } from 'zod';
...
function findContainingArea(table: DBTable, areas: Area[]): Area | null {
    const sortedAreas = [...areas].sort(
        (a, b) => (b.order ?? 0) - (a.order ?? 0)
    );

    return sortedAreas.find((area) => isTableInsideArea(table, area)) ?? null;
}
